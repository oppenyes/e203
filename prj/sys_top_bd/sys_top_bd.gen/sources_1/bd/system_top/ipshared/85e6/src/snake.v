...
//方向控制模块
    ...
    begin
        case (key_out)  //根据开发板上的按键选择对应的难度
            // 4'd12 : begin diff <= 2; state <= 1; end   //困难
            4'hC : begin diff <= 2; state <= 1; end   //困难
            4'hB  : begin diff <= 1; state <= 1; end   //中等
            4'hA  : begin diff <= 0; state <= 1; end   //简单
            default: state <= 0;
        endcase
    end
    else if(state == 1)
    begin
       state <= 2;
        case (diff)   //根据难度去设置相应的速度
            2'd0 : speed <= 24'd1500_0000;
            2'd1 : speed <= 24'd1000_0000;
            2'd2 : speed <= 24'd500_0000;
            default: speed <= 24'd1500_0000;
        endcase        
    end
//state 0 选择难度
//state 1 游戏初始化状态
...
//state 3 死亡状态
//state 4 暂停状态
    else if(state == 3 && key_out == 4'hD)   //在状态3下按S2可以进行新一局的游戏
        state <= 0;
    else if(ishit == 1 || ishit1 == 1)   //如果小蛇死亡，进入状态3，即游戏结束状态
        state <= 3;
    else if(state == 2 && key_out == 4'hE)  //如果处于游戏运行状态，按下S2可以暂停游戏
        state <= 4;
    else if(state <= 4 && key_out == 4'hD)  //如果处于游戏暂停状态，按下S2可以再开始游戏
        state <= 2;
    else
